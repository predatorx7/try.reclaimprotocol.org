export const BACKEND_API_BASE_URL = import.meta.env.VITE_RECLAIM_BACKEND_URL || 'https://api.reclaimprotocol.org';

/**
 * SDK backend Api endpoints
 */
export const SDK_BACKEND_API_ENDPOINTS = {
    APPLICATION: (applicationId: string) => `${BACKEND_API_BASE_URL}/api/applications/info/${applicationId}`,
...
export const fetchApplicationInfo = async (applicationId: string) => {
    const response = await fetch(SDK_BACKEND_API_ENDPOINTS.APPLICATION(applicationId));
    if (response.ok) {
...
export const fetchProviderInfo = async (providerId: string, providerVersion?: string) => {
    const response = await fetch(SDK_BACKEND_API_ENDPOINTS.PROVIDER(providerId, providerVersion));
    if (response.ok) {
