...
const stateColors = {
    idle: ['rgba(191, 84, 123, 0.1)', 'rgba(172, 121, 242, 0.1)', 'rgba(145, 150, 242, 0.1)', 'rgba(182, 197, 242, 0.1)', 'rgba(242, 208, 189, 0.1)'],
    loading: ['rgba(59, 130, 246, 0.05)', 'rgba(96, 165, 250, 0.05)', 'rgba(147, 197, 253, 0.05)', 'rgba(219, 234, 254, 0.05)', 'rgba(37, 99, 235, 0.05)'], // Blues - faster
    success: ['rgba(16, 185, 129, 0.05)', 'rgba(52, 211, 153, 0.05)', 'rgba(110, 231, 183, 0.05)', 'rgba(167, 243, 208, 0.05)', 'rgba(5, 150, 105, 0.05)'], // Greens - slower
    error: ['rgba(239, 68, 68, 0.1)', 'rgba(248, 113, 113, 0.1)', 'rgba(252, 165, 165, 0.1)', 'rgba(254, 226, 226, 0.1)', 'rgba(220, 38, 38, 0.1)'], // Reds - slower
};
...
const liveBackgroundContext = createContext<{ status: LiveBackgroundStatus, setStatus: (status: LiveBackgroundStatus) => void }>({
    status: 'idle',
    setStatus: () => {
        // stub
    }
...
export const LiveBackgroundProvider = ({ children }: { children: React.ReactNode }) => {
    const [status, setStatus] = useState<LiveBackgroundStatus>('idle');
    return <liveBackgroundContext.Provider value={{ status, setStatus }}>{children}</liveBackgroundContext.Provider>;
};
